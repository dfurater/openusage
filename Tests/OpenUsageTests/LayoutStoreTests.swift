    ...
    func testTranslatedDefaultsSeedAnAccountCardTheFirstTimeItAppears() {
        ...
        let registry = WidgetRegistry(
            ...
            descriptors: [
                ...
                descriptor("claude.weekly", claude),
                descriptor("claude.fable", claude),
                descriptor("claude.sonnet", claude),
                descriptor("claude@work.session", work),
                descriptor("claude@work.weekly", work),
                descriptor("claude@work.fable", work),
        ...
        saveStored([
            ...
            PlacedWidget(descriptorID: "claude.fable"),
        ], forKey: "layout", in: defaults)
        defaults.set(
            ["claude.session", "claude.weekly", "claude.fable", "claude.sonnet"],
        ...
        let store = LayoutStore(
            ...
            defaults: defaults,
            storageKey: "layout",
            defaultMetricIDs: ["claude.session", "claude.weekly", "claude.fable"],
            defaultExpandedMetricIDs: ["claude.sonnet"]
        )

        XCTAssertTrue(store.isMetricEnabled("claude@work.session"))
        XCTAssertTrue(store.isMetricEnabled("claude@work.weekly"))
        XCTAssertTrue(store.isMetricEnabled("claude@work.fable"))
        XCTAssertFalse(store.isPinned("claude@work.fable"))
        XCTAssertFalse(store.expandedMetricIDs.contains("claude@work.fable"))
        XCTAssertEqual(store.orderedSupportedMetrics(for: "claude@work").map(\.id), [
            "claude@work.session", "claude@work.weekly", "claude@work.fable", "claude@work.sonnet",
        ])
        XCTAssertFalse(
